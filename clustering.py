...
from clean_process import process_data_WS, stem_tokens, normalize, french_stopwords
import math
import nltk
...
def clean_data_WS(filename):
    """ This function is used for cleaning and sorting the messages scrapped from the CRISP website.
    We sort in 2 categories : the user statements and the responses
    :input: str
        the txt file path where we store the messages
    :output: dataframe
        It returns a table (pandas dataframe) with the columns correspond to respectively the user messages and the responses
        Each row correspond to one conversation (and do one conversation has several messages)
    """
    L = []
    # we open, read and close the file
    ...
    for i in range(1, len(fileLines)):
        ...
        if line != '\n':
            # if the line starts with 'Conversation', it means that we have switched to a new conversation and we creat a new line in the table
            if line.startswith("Conversation"):
                ...
                # case if the message has been splitted in 2 or more parts
                else:
